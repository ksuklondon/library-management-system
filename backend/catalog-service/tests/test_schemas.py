...
class TestBookSchemas:
    ...
    def test_book_create_isbn_validation(self):
        ...
        # ISBN z literami
        with pytest.raises(ValidationError) as exc_info:
            BookCreate(title="Test", authors="Test", isbn="123ABC7890")
    ...
    def test_book_create_isbn_10_valid(self):
        ...
        book = BookCreate(title="Test", authors="Test", isbn="1234567890")

        assert book.isbn == "1234567890"

    def test_book_create_isbn_13_valid(self):
        """Test poprawnego ISBN-13."""
        book = BookCreate(title="Test", authors="Test", isbn="1234567890123")
    ...
    def test_book_create_missing_required_fields(self):
        ...
        with pytest.raises(ValidationError):
            BookCreate(authors="Test Author")
    ...
    def test_book_update_partial(self):
        ...
        # Tylko title
        update1 = BookUpdate(title="New Title")
        assert update1.title == "New Title"
        assert update1.authors is None

        # Tylko pages
        update2 = BookUpdate(pages=500)
        assert update2.pages == 500
        assert update2.title is None

    def test_book_response_from_orm(self, sample_book, sample_book_copy):
...
class TestBookCopySchemas:
    ...
    def test_book_copy_create_missing_required(self):
        ...
        # Brak inventory_no
        with pytest.raises(ValidationError):
            BookCopyCreate(book_id=uuid.uuid4())

        # Brak book_id
        with pytest.raises(ValidationError):
            BookCopyCreate(inventory_no="INV-001")

    def test_copy_status_update_valid(self):
        """Test zmiany statusu egzemplarza (F16)."""
        status_update = CopyStatusUpdate(status=CopyStatusEnum.DAMAGED)

        assert status_update.status == CopyStatusEnum.DAMAGED

    def test_copy_status_enum_values(self):
        """Test wszystkich wartości enum CopyStatus."""
        assert CopyStatusEnum.AVAILABLE == "AVAILABLE"
        assert CopyStatusEnum.RESERVED == "RESERVED"
        assert CopyStatusEnum.BORROWED == "BORROWED"
        assert CopyStatusEnum.DAMAGED == "DAMAGED"
        assert CopyStatusEnum.LOST == "LOST"

    def test_book_copy_update_partial(self):
        """Test częściowej aktualizacji egzemplarza."""
        # Tylko location
        update1 = BookCopyUpdate(location="New Location")
        assert update1.location == "New Location"
        assert update1.inventory_no is None

        # Tylko status
        update2 = BookCopyUpdate(status=CopyStatusEnum.DAMAGED)
        assert update2.status == CopyStatusEnum.DAMAGED
...
class TestCatalogSchemas:
    ...
    def test_search_query_valid(self):
        ...
        query = SearchQuery(query="Python", search_in="title")
    ...
    def test_search_query_min_length(self):
        ...
        with pytest.raises(ValidationError):
            SearchQuery(query="")  # Puste zapytanie
    ...
    def test_catalog_filter_all_fields(self):
        ...
        assert filter_data.available_only == True
    ...
    def test_catalog_filter_optional_fields(self):
        ...
        assert filter_data.available_only == False
    ...
    def test_pagination_params_max_page_size(self):
        ...
        with pytest.raises(ValidationError):
            PaginationParams(page_size=100)  # Ponad 50!

    def test_pagination_params_min_page(self):
        """Test minimalnego numeru strony."""
        with pytest.raises(ValidationError):
            PaginationParams(page=0)  # Strona musi być >= 1

    def test_pagination_get_offset(self):
        """Test obliczania offset dla paginacji."""
        # Strona 1, rozmiar 20
        pagination1 = PaginationParams(page=1, page_size=20)
        assert pagination1.get_offset() == 0

        # Strona 2, rozmiar 20
        pagination2 = PaginationParams(page=2, page_size=20)
        assert pagination2.get_offset() == 20

        # Strona 3, rozmiar 10
        pagination3 = PaginationParams(page=3, page_size=10)
